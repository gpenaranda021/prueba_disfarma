...
<html lang="en">
  <head>
    ...
    <title>Listar ventas | Prueba Disfarma</title>
...
<main role="main" class="container">
  ...
  <div class="starter-template">
    <h1>Listar ventas</h1>
    <p class="lead">A continuación, se listan las ventas registradas en la base de datos.</p>
  </div>
...
$url="http://localhost/prueba_disfarma/api/venta/leer_venta.php";
$data_sol=array();
...
if(isset($resp["response"]))
{
    ...
    foreach($data AS $d)
    {
        ...
        '<tr>
            ...
            if($d["payment_mode"]=="EF"){ echo '<td>'.$d["payment_mode"].'-Efectivo</td>';}
            if($d["payment_mode"]=="TC"){ echo '<td>'.$d["payment_mode"].'-Tarjeta Crédito</td>';}
            if($d["payment_mode"]=="TD"){ echo '<td>'.$d["payment_mode"].'-Tarjeta Débito</td>';}
            if($d["payment_mode"]=="TF"){ echo '<td>'.$d["payment_mode"].'-Transferencia</td>';}
        echo '</tr>';
        //echo $d["id"];
        //echo $d["name"];
    }
